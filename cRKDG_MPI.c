...
void output() {
    ...
    for (i = 0; i < Nx; i++) {
        fprintf(fp, "%d ", i);
        for (j = 0; j <dimPK; j++) {
            fprintf(fp, "%.15e ", params.uh[i][j]);  // 使用科学计数法，减少精度
        }
        fprintf(fp, "\n");
    }
    fclose(fp);
...
void Lh_loc(double uhx[Nx][dimPK], double du1[Nx][dimPK], int idx, int N, int rank, int size) {
    int i, j, s;
    ...
    // 局部数组定义
    double local_uhG[N][NumGLP];
    double local_uhR[N][1];
    double local_uhL[N][1];
    double local_flat[N][2];
    ...
    for(i = 0; i <N; i++) {
        for(j = 0; j < dimPK; j++) {
            local_uhR[i][0] += uhx[idx+i][j] * params.phigr[j];
            local_uhL[i][0] += uhx[idx+i][j] * params.phigl[j];
        }
    }
    
    for(i = 0; i < N; i++) {
        for(j = 0; j <dimPK;j++) {
             params.test[idx+i][0] = local_uhR[i][0];  // 保存uhx到test数组
             params.test[idx+i][1] = local_uhL[i][0];  // 保存uhx到test数组       
        }
    }
    ...
    for(i = 0; i < N; i++) {
        for(j = 0; j < dimPK; j++) {
            du1[idx+i][j] -= (1.0/params.hx) * 
                (params.phigr[j] * func(local_uhR[i][0]) - params.phigl[j]*func(local_uhL[i][0]));
        }
...
void RK3(int idx, int N, int rank, int size) {
    ...
    while (params.t < params.tend) {
        ...
        // RK3 第一步
        Lh_loc(params.uh, params.du2, idx, N, rank, size);
        ...
        // RK3 第二步
        Lh_loc(params.uh1, params.du2, idx, N, rank, size);
        ...
        Lh(params.uh, params.du, idx, N, rank, size);
        // 同步所有进程的du2数据
        MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, 
                      params.du2, N * dimPK, MPI_DOUBLE, MPI_COMM_WORLD);
